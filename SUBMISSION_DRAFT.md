...
## Editorial notes for Mai's review

- This draft leans toward "long and thorough" per the brief — trimming for the actual Google Doc is expected.
- Section 4 is the longest section by design, since the assignment brief and the source material both treat it as the most valuable part.
- ~~I did not include the full Phase 4 delivery-channel saga...~~ Resolved: given its own subsection in Section 4 ("The manual-round-trip approach itself: an ineffective detour, not just a delivery problem"), per Mai's review — it now covers both the three-attempt delivery failure and the deeper point that the round-trip approach was the wrong one, plus the plan-review gap that let it run two sessions before the scope error was caught.
- I left out most of the granular Phase 2/3 UI-bug fixes (Vega-Lite label overlap, dashed-segment rendering gaps, CSS truncation on Key Insights cards) — they're real and documented in BUILD_LOG.md Entries 4 and 6.5, but felt like implementation detail rather than "iteration" in the sense the assignment is asking about. Flag if you want a "notable bugs fixed" appendix.
- ~~The Prompts section is a subset...particularly around the Phase 5 taxonomy revision rounds...~~ Resolved: added a "Phase 5 taxonomy revision rounds" subsection (6 more prompts) covering the egg-category pushback, the pivotal "aligned and misaligned" self-correction, the no-API-key cost check, the twelve-drift-point plan verification, the composition-not-destination taxonomy corrections, and the closing manual-review round — same story-arc curation style as the rest of the section.
