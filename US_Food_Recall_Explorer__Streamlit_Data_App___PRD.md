...
* **Filtering & Selection (Priority: High)**
  ...
  * **Trend charts must render the current partial year as a dashed, explicitly labelled segment** ("partial year — through July 2026"). The label uses the last actual `recall_initiation_date` (2026-07-08), not openFDA's `openfda_last_updated` metadata timestamp (2026-08-05) — that date reflects when the source was published, not the last recall in it, and using it would overstate the snapshot's coverage by about a month. Either way, 2026 holds ~7 months against every other year's 12 and plots 692 against 2025's 1,571 — a phantom 56% drop. Annualising or projecting the year is explicitly forbidden: it invents data and edges into the predictive framing the Non-Goals rule out. This is the one permitted exception to the no-per-chart-annotation rule, because the misreading happens at the chart and cannot be prevented from inside a collapsed About section. (Confirmed in Phase 2, Slice 1 — see `HANDOFF_PHASE2_SLICE2.md`.)
...
> - **2026 is a partial year** (last actual recall date 2026-07-08; the snapshot's 2026-08-05 metadata date is when openFDA's source was fetched/published, not the last recall in it). The final point on every trend line will dip artificially and needs explicit visual treatment — a dashed segment or annotation — not a footnote.
